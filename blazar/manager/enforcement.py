...
class UsageEnforcer(object):
    ...
    def _get_project_enforcement_id(self, project_id):
        ...
        try:
            ...
            LOG.info('Enforcement_attribute: '+CONF.enforcement.project_enforcement_id+'. Project: '+enforcement_attribute)
            if not enforcement_attribute:
                msg = 'Enforcement attribute project_enforcement_id {} is null in keystone.'
                msg.format(CONF.enforcement.project_enforcement_id)
                LOG.info(msg)
                raise ValueError(msg)
        except (AttributeError, TypeError):
            LOG.info('/etc/blazar/blazar.conf: Parameter project_enforcement_id in group enforcement missing or unsupported. Using project_name.')
